...
//--------------------------------------------------------------------------------------
// THIS UART STRUCT CREATES STRUCTS TO DEFINE THE UART CONFIGURATION FOR LATER USE,
// THESE STRUCTS CAN NOW BE USED IN FUNCTIONS AND PASSED INTO OTHER FUNCTIONS IN OTHER CLASSES
struct _UART
{
    UART_Config config;
...
//       implemented.
//---------------------------------------------------------------------------------------
// this function gets the character from the UART receiver Buffer
char UART_getChar(UART* uart_p);
// this function confirms that a character has been received
bool UART_hasChar(UART* uart_p);
// this function confirms the connection between both devices is still present
// available to send data
bool UART_canSend(UART* uart_p);
// this function puts the character that we wish to sent in the sending buffer
// as well as checking if the sending buffer has space to send characters
